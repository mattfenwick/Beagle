...
var rootDataNode = Object.create({}),
    types = {},
    constructors = {};

function makeType(typename, argnames) {
    var type = Object.create(rootDataNode, {'datatype': {'value': typename}}), // TODO can I make this be enumerated?
...
var functions = (function() {
    ...
    var plus = constructors.BuiltinFunc(
        ...
        ['Number', 'Number'],
        function(args) {
            return constructors.Number(args[0].numValue + args[1].numValue);
        }
    );
...
function applyFunction(func, env, args) {
    ...
    if (func.datatype === 'UserFunc') {
        ...
        if (args.length !== func.params.length) {
            throw new Error('incorrect number of arguments to user-defined function');
        }
        // put parameter bindings into local environment
        for (var j = 0; j < func.params.length; j++) {
            newEnv.addBinding(func.params[j], args[j]);
        }
        ...
        if (evaledArgs.length !== func.types.length) { // TODO should check this *before* evaluating arguments
            throw new FunctionError("NumArgsError", func.types.length, evaledArgs.length, func.name, '');
        }
...
module.exports = {
    ...
    // TODO types, constructors, root data node
    'actions'      :  actions,
};
