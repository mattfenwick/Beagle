...
// TODO what about def/set in conditionals?  or used as expressions?
/*
var typeDefs = [
    ['Number',      ['numValue'             ]],
    ['Symbol',      ['strValue'             ]],
    ['String',      ['strValue'             ]], // TODO should this be changed to 'chars'? (since it's not a string)
    ['List',        ['elems'                ]],
    ['Dictionary',  ['keyvals'              ]],
...
function symbol(node, state, env, log) {
    ...
    if (!env.hasBinding(name)) {
        ...
        env.setBinding(env.getBinding(name) + 1);
    }
}
...
function fn(node, state, env, log) {
    ...
    node.params.forEach(function(p) {
        if (newEnv.getBinding(p.strValue) === 0) {
            log.warnings.push({'name': name, 'position': p._debug.start, 'message': 'unused variable'});
        }
    });
}
...
function variableUsage(node) {
    ...
    */
    ...
    var state = {},
        env = environment.Environment(null, bindings),
        log = {'errors': [], 'warnings': []};
